...
const FAQS = [
  {
    ...
    answer: "Dès la réception de votre commande, notre équipe analyse votre dossier et travaille dessus directement. Nous nous engageons à vous fournir le site web en 30 secondes."
  },
  {
  ...
  return (
    <section id="faq" style={{ paddingTop: 80, paddingBottom: 80 }}>
      <div style={{ maxWidth: 900, margin: '0 auto', paddingLeft: 40, paddingRight: 40 }}>
            ...
            {FAQS.map((faq, i) => (
              <div key={i} style={{ borderBottom: '1px solid #1f1f1f' }}>
                <button
                  ...
                  style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, paddingTop: 18, paddingBottom: 18, background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}
                >
                  <span style={{ fontSize: 15, color: open === i ? '#AAFF00' : '#ffffff', fontWeight: 500, transition: 'color 200ms' }}>{faq.question}</span>
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ flexShrink: 0, transform: open === i ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 200ms' }}>
        ...
        {/* Desktop — 2 colonnes */}
        <div className="hidden md:flex" style={{ gap: 60, alignItems: 'flex-start' }}>
